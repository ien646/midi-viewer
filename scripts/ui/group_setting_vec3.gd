...
func _handle_config_value_changed(section: String, key: String, value: Variant):
	if(section.begins_with(config_section) && section.ends_with(Utils.group_tag(group)) && key == config_key):
...
func _handle_x_value_changed(value: float):
	config_node.update_config_value_tagged(config_section, Utils.group_tag(group), config_key, Vector3(value, y_node.value, z_node.value));
	
func _handle_y_value_changed(value: float):
	config_node.update_config_value_tagged(config_section, Utils.group_tag(group), config_key, Vector3(x_node.value, value, z_node.value));
	
func _handle_z_value_changed(value: float):
	config_node.update_config_value_tagged(config_section, Utils.group_tag(group), config_key, Vector3(x_node.value, y_node.value, value));
	
func _ready() -> void:
	var value = Config.get_tagged_value(config_section, config_key, Utils.group_tag(group)) as Vector3;
